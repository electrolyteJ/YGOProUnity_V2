    ...
    public static class Config
    {
        ...
        private static Dictionary<string, string> LoadArgs(string[] args)
        {
            ...
            for (int i = 0; i < args.Length; ++i)
            {
                ...
                if (position == -1)
                    throw new System.Exception("Invalid argument '" + option + "': no key/value separator");

                string key = option.Substring(0, position).Trim().ToUpper();
                string value = option.Substring(position + 1).Trim();

                if (fields.ContainsKey(key))
                    throw new System.Exception("Invalid argument '" + option + "': duplicate key '" + key + "'");
        ...
        private static Dictionary<string, string> LoadFile(string filename)
        {
            ...
            // Unity: try StreamingAssets first, then dataPath
            string streamingPath = System.IO.Path.Combine(
                UnityEngine.Application.streamingAssetsPath, filename);
            ...
            {
                Logger.WriteErrorLine("Config file not found: " + filename);
                return fields;
            }
            using (System.IO.StreamReader reader = new System.IO.StreamReader(filePath))
            {
                ...
                while (!reader.EndOfStream)
                {
                    ...
                    if (position == -1)
                        throw new System.Exception("Invalid configuration file: no key/value separator line " + lineNumber);
        
                    string key = line.Substring(0, position).Trim().ToUpper();
                    string value = line.Substring(position + 1).Trim();
        
                    if (fields.ContainsKey(key))
                        throw new System.Exception("Invalid configuration file: duplicate key '" + key + "' line " + lineNumber);
